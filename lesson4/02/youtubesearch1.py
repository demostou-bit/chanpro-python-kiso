...
class YoutubeSearch(tk.Frame):
	...
	def __init__(self, master=None):
		...
		# APIキーを設定
		self.apikey = "xxxxxxxxxxxx APIキー xxxxxxxxxxxxxx"
		...
		# 結果表示用のフレーム
		self.results_frame = tk.Frame(self, bg="lightblue", pady=2)
		self.results_frame.pack(fill=tk.Y)
	...
	def search(self):
		...
		# キーワードの入力がない場合にはエラー
		if len(keyword) == 0:
			tk.messagebox.showerror("エラー", "検索キーワードを入力してください")
			return

		# クエリパラメータ
		params = {"part": "snippet", "q": keyword, "type": "video",
							"maxResults": "6", "key": self.apikey}

		# リクエストを送信して結果を取得
		results = requests.get(self.youtube_api, params=params).json()

		if len(results["items"]) == 0:
			tk.messagebox.showerror("エラー", "見つかりません")
			return

		# 検索結果を一つずつ取り出しContentFrameを生成して表示する
		for item in results["items"]:
			if "videoId" in item["id"]:
				...
				self.search_results.append(ContentFrame(
					self.results_frame, id, title, desc, img))
